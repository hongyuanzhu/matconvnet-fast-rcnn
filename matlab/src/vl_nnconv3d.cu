// @file nnconv3d.cu
// @brief 3D Convolution block MEX wrapper
// @author Tuan-Hung Vu
                 ...
                 int nin, mxArray const *in[])
{
      ...
      case opt_stride :
        ...
        switch (mxGetNumberOfElements(optarg)) {
          ...
          case 3:
            ...
            strideT = (int)mxGetPr(optarg)[2] ;
            break ;
          default:
            mexErrMsgTxt("STRIDE has neither one nor three elements.") ;
        }
        break ;
  ...
  if (strideX < 1 || strideY < 1 || strideT < 1) {
  ...
  vl::TensorShape filtersGeom(filters) ;
  int equivalentNumFilters ;
  if (hasFilters) {
    if (filtersGeom.getDimension(0) == 0 || filtersGeom.getDimension(1) == 0 || filtersGeom.getDimension(2) == 0 || filtersGeom.getDimension(3) == 0) {
    ...
    if (data.getDimension(0) + (padTop+padBottom) < filters.getDimension(0) ||
        ...
        data.getDimension(2) + (padT+padT) < filters.getDimension(2)) {
    ...
    filtersGeom.clear();
    filtersGeom.setDimension(0, 1);
    filtersGeom.setDimension(1, 1);
    filtersGeom.setDimension(2, 1);
    filtersGeom.setDimension(3, data.getDimension(3));
    filtersGeom.setDimension(4, data.getDimension(3));

    numFilterGroups = 1 ;
    equivalentNumFilters = data.getDimension(3) ;
  }

  /* Get the output geometry */
  vl::TensorShape outputGeom;
  outputGeom.setDimension(0, (data.getDimension(0) + (padTop+padBottom) - filtersGeom.getDimension(0))/strideY + 1);
  outputGeom.setDimension(1, (data.getDimension(1)  + (padLeft+padRight) - filtersGeom.getDimension(1))/strideX + 1);
  outputGeom.setDimension(2, (data.getDimension(2)  + (padT+padT) - filtersGeom.getDimension(2))/strideT + 1);
  outputGeom.setDimension(3, equivalentNumFilters);
  outputGeom.setDimension(4, data.getDimension(4));
  ...
  if (hasBiases) {
    if (biases.getNumElements() != filtersGeom.getDimension(4)) {
  ...
  /* untested
   ...
   the output is 1 x 1 x 1 pixels,
   no padding,
   one filter group,
   stride of one pixel
   */
  // fullyConnectedMode = (outputGeom.getDimension(0) == 1 &&
  //                       outputGeom.getDimension(1) == 1 &&
  //                       outputGeom.getDimension(2) == 1 &&
  //                       strideY == 1 &&
  //                       strideX == 1 &&
  //                       strideT == 1 &&
  //                       padTop == 0 &&
  ...
  if (verbosity > 0) {
    mexPrintf("vl_nnconv3d: %s; %s", backMode?"backward":"forward", (data.getDeviceType()==vl::GPU) ? "GPU" : "CPU") ;
    ...
    mexPrintf("vl_nnconv3d: stride: [%d %d %d], pad: [%d %d %d %d %d]\n"
              "vl_nnconv3d: num filter groups: %d, has bias: %d, has filters: %d, is fully connected: %d\n",
              strideY, strideX, strideT,
              padTop, padBottom, padLeft, padRight, padT,
              numFilterGroups, hasBiases, hasFilters, fullyConnectedMode) ;
    vl::print("vl_nnconv3d: data: ", data) ;
    if (hasFilters) { vl::print("vl_nnconv3d: filters: ", filters) ; }
    if (hasBiases) { vl::print("vl_nnconv3d: biases: ", biases) ; }
    if (backMode) {
      vl::print("vl_nnconv3d: derOutput: ", derOutput) ;
      vl::print("vl_nnconv3d: derData: ", derData) ;
      if (hasFilters) { vl::print("vl_nnconv3d: derFilters: ", derFilters) ; }
      if (hasBiases) { vl::print("vl_nnconv3d: derBiases: ", derBiases) ; }
    } else {
      vl::print("vl_nnconv3d: output: ", output) ;
    }
  }
  ...
  /* regular case */
  if (!backMode) {
    error = vl::nnconv3d_forward(context,
                               output, 0,
                               data, 1,
                               filters,
                               biases,
    ...
    error = vl::nnconv3d_backward(context,
                                derData,
                                derFilters,
                                derBiases,
                                data,
  ...
  if (verbosity > 0) {
    ...
    if (context.getCudaHelper().getCudnnEnabled()) {
      mexPrintf("vl_nnconv3d: cuDNN workspace used: "
                "fwd %.2f MB"
                ", bwd filter %.2f MB"
                ", bwd data %.2f MB\n",
